...
export default function QuoteModal({ open, onClose }: QuoteModalProps) {
  ...
  const onSubmit = async (data: FormData) => {
    setStatus('loading')
    try {
      const cartItems = items.map((i) => ({ productName: i.nombre, codigo: i.codigo, quantity: i.quantity }))
        ...
        <div className="bg-[#F2F2F0] rounded-xl p-3 mb-4">
          {items.map((i) => (
            <div key={i.codigo} className="flex justify-between text-xs py-1">
              <span className="text-[#0A0A0A]">{i.nombre}</span>
