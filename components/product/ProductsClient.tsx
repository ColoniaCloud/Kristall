...
// eso, gana siempre.
const selectTrigger = 'flex-1 min-w-0 border-transparent text-sm font-medium data-placeholder:text-[#5C5C5C]'
const triggerStyle = { height: 40, backgroundColor: '#F2F2F0' }
// En mobile el desplegable ocupa casi todo el ancho de pantalla (más fácil de
// tocar con 17 líneas en la lista); en desktop, el ancho default de Radix
// (alineado al trigger) ya andaba bien y no se toca.
const selectContent = 'border border-white/10 w-[calc(100vw-2rem)] md:w-auto'
...
export default function ProductsClient() {
  ...
  // El ítem "all" se renderiza aparte, con el nombre del filtro como label
  // (ver JSX) en vez de "Todos" — estas listas arrancan directo en los valores reales.
  const VLT_OPTIONS = [
          ...
          <div className="flex flex-row gap-2 md:gap-3">
            <Select value={activeLinea} onValueChange={setActiveLinea}>
              ...
              <SelectContent className={selectContent} style={contentStyle}>
                <SelectItem className={selectItem} value="all">{t('filter_linea')}</SelectItem>
                {LINEAS.map((linea) => (
            ...
            <Select value={activeVLT} onValueChange={setActiveVLT}>
              ...
              <SelectContent className={selectContent} style={contentStyle}>
                <SelectItem className={selectItem} value="all">{t('filter_vlt')}</SelectItem>
                {VLT_OPTIONS.map((opt) => (
            ...
            <Select value={activeUV} onValueChange={setActiveUV}>
              ...
              <SelectContent className={selectContent} style={contentStyle}>
                <SelectItem className={selectItem} value="all">{t('filter_uv')}</SelectItem>
                {UV_OPTIONS.map((opt) => (
                  <SelectItem className={selectItem} key={opt.value} value={opt.value}>{opt.label}</SelectItem>
                ))}
              </SelectContent>
