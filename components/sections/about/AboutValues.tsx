        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {smallCards.map(({ Icon, titleKey, bodyKey, dark }) => (
            ...
            >
              ...
              <div className={`text-[13px] font-medium mb-1 ${dark ? 'text-white' : 'text-[#0A0A0A]'}`}>{t(titleKey as Parameters<typeof t>[0])}</div>
              <div className={`text-[11px] leading-relaxed ${dark ? 'text-white/45' : 'text-[#9A9A9A]'}`}>{t(bodyKey as Parameters<typeof t>[0])}</div>
            </div>
          ))}
        </div>
      </div>
